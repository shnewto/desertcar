...
fn _gamepad_movement(
    ...
    if let Ok((mut movements, mut transform)) = query.get_single_mut() {
        ...
        let right_trigger = button_axes
            ...
            .unwrap();
        let push_factor = if right_trigger.abs() > 0.01 {
            19.0
        } else {
            13.0
        };
        ...
        if let (Some(x), Some(y)) = (axes.get(axis_rx), axes.get(axis_ry)) {
            ...
            if stick_pos.length() > 0.9 && stick_pos.y > 0.5 {
                movements
                    .0
                    .push(Movement::PushForward(push_factor * stick_pos.y.abs()))
            }

            if stick_pos.length() > 0.9 && stick_pos.y < 0.5 {
                movements
                    .0
                    .push(Movement::PushBackward(push_factor * stick_pos.y.abs()))
            }

            if stick_pos.length() > 0.9 && stick_pos.x < 0.5 {
                movements
                    .0
                    .push(Movement::TurnRight(turn_factor * stick_pos.x.abs()))
            }

            if stick_pos.length() > 0.9 && stick_pos.x > 0.5 {
                movements
                    .0
                    .push(Movement::TurnLeft(turn_factor * stick_pos.x.abs()))
            }
        }
